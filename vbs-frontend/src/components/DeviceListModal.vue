...
async function toggleDeviceHistory(deviceId: string, deviceName: string, poolId: string, poolName: string) {
  // 如果点击的是当前已展开的设备，则关闭
  ...
  const history = await store.getDeviceStatusHistory(poolId, deviceId, selectedDeviceTimeRange.value);
  if (history) {
...
async function onDeviceTimeRangeChange(hours: number) {
  ...
  if (activeDeviceId.value) {
    ...
    if (device) {
      const history = await store.getDeviceStatusHistory(device.poolId, activeDeviceId.value, hours);
      if (history) {
        selectedDeviceHistory.value = history;
      }
    }
