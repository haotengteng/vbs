...
const defaultColors = ['#00d4ff', '#00ccff', '#0099ff', '#0066cc'];

const runningDevices = computed(() => store.dashboardStats?.runningDevices ?? 27);
const faultDevices = computed(() => store.dashboardStats?.faultDevices ?? 2);
...
const deviceCategories = computed<DeviceCategory[]>(() => {
  if (store.dashboardStats?.deviceCategories?.length) {
    return store.dashboardStats.deviceCategories.map((c, i) => ({
      name: c.name,
      value: c.value,
