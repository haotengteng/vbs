...
const totalDuration = computed(() => {
  ...
  const first = new Date(props.history.records[0].startTime);
  const last = new Date(props.history.records[props.history.records.length - 1].endTime);
  return last.getTime() - first.getTime();
});

const startTime = computed(() => {
  if (!props.history || props.history.records.length === 0) return null;
  return new Date(props.history.records[0].startTime);
});

const endTime = computed(() => {
  if (!props.history || props.history.records.length === 0) return null;
  return new Date(props.history.records[props.history.records.length - 1].endTime);
});
      ...
      <Transition name="tooltip">
        ...
        >
          ...
          <div class="tooltip-row">
            ...
            <span class="tooltip-value">{{ formatDuration(new Date(hoverRecord.endTime).getTime() - new Date(hoverRecord.startTime).getTime()) }}</span>
          </div>
        </div>
      </Transition>
    </Teleport>
